...
async function ensureAdminProfile(db, user) {
  ...
  const merged = { ...data };
  const isAdmin =
    merged.isAdmin === true || (typeof merged.role === 'string' && merged.role.toLowerCase() === 'administrator');
  ...
  if (isAdmin && Array.isArray(merged.segments) && !merged.segments.includes('Admin')) {
    ...
    try {
      await updateDoc(userRef, { segments: merged.segments, updatedAt: serverTimestamp() });
    } catch (error) {
      console.warn('Unable to sync admin segments', error);
    }
  }

  if (isAdmin && merged.vipTier !== 'Executive') {
    try {
      await updateDoc(userRef, { vipTier: 'Executive', updatedAt: serverTimestamp() });
      merged.vipTier = 'Executive';
    } catch (error) {
      console.warn('Unable to sync admin vip tier', error);
    }
  }

  if (isAdmin && (typeof merged.role !== 'string' || merged.role.toLowerCase() !== 'administrator')) {
    try {
      await updateDoc(userRef, { role: 'Administrator', updatedAt: serverTimestamp() });
      merged.role = 'Administrator';
    } catch (error) {
      console.warn('Unable to sync admin role', error);
    }
  }
